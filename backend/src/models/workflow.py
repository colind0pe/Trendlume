...
class JobEventModel(Base):
    ...
    job_id: Mapped[str] = mapped_column(String(36), ForeignKey("workflow_jobs.id", ondelete="CASCADE"), nullable=False)
...
class WorkflowStepRunModel(Base):
    ...
    __table_args__ = (
        UniqueConstraint("task_id", "step_key", "unit_key", "attempt", name="uq_workflow_step_attempt"),
    ...
    reused_from_id: Mapped[str | None] = mapped_column(ForeignKey("workflow_step_runs.id"), nullable=True)
...
class WorkflowArtifactModel(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True)
    task_id: Mapped[str] = mapped_column(ForeignKey("tasks.id", ondelete="CASCADE"))
    step_run_id: Mapped[str] = mapped_column(ForeignKey("workflow_step_runs.id", ondelete="CASCADE"))
...
class WorkflowStepArtifactModel(Base):
    ...
    step_run_id: Mapped[str] = mapped_column(ForeignKey("workflow_step_runs.id", ondelete="CASCADE"), primary_key=True)
    artifact_id: Mapped[str] = mapped_column(ForeignKey("workflow_artifacts.id", ondelete="NO ACTION"), primary_key=True)
