...
import {
  generationStatus,
  isGenerationLifecycleEvent,
  isSourceMaterialMode,
} from "@/lib/task-generation-state";
...
const researchContextForScript = (research: ResearchResponse | null) => {
  ...
  return [
    ...
    ...research.sources.map(
      (source, index) =>
        `[${source.ref_id || `source-${index + 1}`}] ${source.title}\nURL: ${source.url}\n摘要：${source.snippet}`,
    ),
  ].join("\n");
};
...
export default function TaskStoryboardPage() {
  ...
  const { data: task, isLoading: isTaskLoading } = useQuery({
    ...
    refetchInterval: (query) => {
      const currentTask = query.state.data as
        ...
        | undefined;
      const activeStatus =
        currentTask?.active_job?.status || currentTask?.status;
      return ACTIVE_TASK_STATUSES.has(activeStatus || "") ? 2000 : false;
    },
  });
  ...
  const categoryStatus = (type: string) =>
    configSummary?.categories.find((item) => item.type === type)?.status ||
    "not_configured";
  ...
  const searchConfigured = !["not_configured", "disabled"].includes(
    searchStatus,
  );

  const configuredContentMode =
    task?.generation_settings?.content_mode || "generated_image";
  ...
  const isGeneratedImageMode =
    configuredContentModeSpec?.sourceKind === "ai" &&
    configuredContentModeSpec.visualKind === "image";
  const isGeneratedVideoMode =
    configuredContentModeSpec?.sourceKind === "ai" &&
    configuredContentModeSpec.visualKind === "video";
    ...
    ? [
        {
          ...
          solution:
            "当前为素材库视频模式，流水线将自动检索实拍视频；也可在各分镜中绑定我的素材。",
        },
      ]
      ...
      ? [
          {
            ...
            solution:
              "请配置视频生成 Provider，或新建任务时改用 AI 图片、素材库视频或我的素材。",
          },
        ]
  ...
  const readinessItems = [
    ...
    },
    {
      ...
      solution:
        "AI 剧本生成将自动跳过联网搜索，直接基于语言模型已有知识创作，不阻塞流程。",
    },
    ...visualReadinessItems,
    {
      ...
      solution:
        "可手动上传解说录音，或在设置中配置语音合成 Provider (Edge-TTS)。",
    },
    {
      type: "publishing",
      title: "抖音社交发布",
  ...
  const [retryingUnits, setRetryingUnits] = React.useState<
  ...
  const [missingConfigAlert, setMissingConfigAlert] = React.useState<
    string | null
  >(null);
  ...
  const [renderTemplateId, setRenderTemplateId] = React.useState(
  ...
  const [liveStepMessage, setLiveStepMessage] = React.useState<string | null>(
    null,
  );
  const [liveCurrentScene, setLiveCurrentScene] = React.useState<number | null>(
    null,
  );
  const [liveTotalScenes, setLiveTotalScenes] = React.useState<number | null>(
    null,
  );
  ...
  const [isApplyScriptConfirmOpen, setIsApplyScriptConfirmOpen] =
    React.useState(false);
  ...
  const [researchData, setResearchData] =
    React.useState<ResearchResponse | null>(null);
  const [generatedScript, setGeneratedScript] =
    React.useState<StructuredScript | null>(null);
  ...
  const [publishMode, setPublishMode] = React.useState<"now" | "schedule">(
    "now",
  );
  const [scheduledAt, setScheduledAt] = React.useState("");
  const [isRerenderOpen, setIsRerenderOpen] = React.useState(false);
  const [rerenderTemplateId, setRerenderTemplateId] = React.useState(
    "image_gallery_matted",
  );
  const [rerenderParams, setRerenderParams] = React.useState<
    Record<string, any>
  >({});
  ...
  const [generatingTTS, setGeneratingTTS] = React.useState<
    Record<string, boolean>
  >({});
  const [generatingImage, setGeneratingImage] = React.useState<
    Record<string, boolean>
  >({});
  const [generatingVideo, setGeneratingVideo] = React.useState<
    Record<string, boolean>
  >({});
  ...
  React.useEffect(() => {
    if (
      ...
      resetLiveState();
  }, [task?.production_status, task?.updated_at, resetLiveState]);

  React.useEffect(() => {
    if (!task || isDirtyRef.current) return;

    const inputPayload = task.generation_settings || {};
    const generatedMetadata = inputPayload.metadata as
    ...
    if (task) {
      ...
      setPubTags(
        Array.isArray(generatedMetadata?.tags) &&
      ...
      if (Number.isFinite(configuredSceneCount)) {
        setAiSceneCount(
          Math.min(
            SCENE_COUNT_MAX,
            Math.max(SCENE_COUNT_MIN, Math.round(configuredSceneCount)),
          ),
        );
      }
      if (task.scenes && task.scenes.length > 0) {
        setScenes(
          task.scenes.map((s, idx) => ({
            sequence_index:
              s.sequence_index !== undefined ? s.sequence_index : idx,
  ...
  const handleTaskEvent = React.useCallback(
    (event: TaskEvent) => {
      ...
      if (typeof data.current_scene === "number")
        setLiveCurrentScene(data.current_scene);
      if (typeof data.total_scenes === "number")
        setLiveTotalScenes(data.total_scenes);
      ...
      const researchStageEvent =
        ...
        ((event.event === "step.started" || event.event === "step.completed") &&
          stage === "research");
  ...
  const handleRetryWorkflowUnit = async (
    stepKey: "voice" | "assets",
    unitKey: string,
  ) => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["task-detail", taskId] });
      toast(
        `已触发分镜 ${stepKey === "voice" ? "配音" : "画面"} 重新生成（最小重算）`,
        "success",
      );
    } catch (err: any) {
  ...
  const regenerateMetadataMutation = useMutation({
    ...
    onError: (err: unknown) =>
      toast(`发布信息生成失败：${errorMessage(err, "请稍后重试")}`, "error"),
  });
  ...
  const generateWorkflowMutation = useMutation({
    ...
    onError: (err: any) => {
      resetLiveState();
      setMissingConfigAlert(
        err.message || "启动全自动生成失败，请检查 AI 模型凭证配置。",
      );
    },
  });
  ...
  const retryWorkflowMutation = useMutation({
    ...
    onError: (err: any) => {
      resetLiveState();
      setMissingConfigAlert(
        err.message || "重试生成失败，请检查相关 AI 配置。",
      );
    },
  });
  ...
  const composeVideoMutation = useMutation({
    ...
    onError: (err: any) => {
      setMissingConfigAlert(
        err.message || "成片合成失败，请检查分镜素材与 FFmpeg 环境。",
      );
    },
  });
  ...
  const scheduleMutation = useMutation({
    ...
    onError: (err: any) =>
      toast(
        `定时发布失败：${errorMessage(err, "请检查排期时间和账号状态")}`,
        "error",
      ),
  });
  ...
  const cancelScheduledPublishMutation = useMutation({
    ...
    onError: (err: any) =>
      toast(`取消自动发布失败：${errorMessage(err, "请稍后重试")}`, "error"),
  });

  const resumeMutation = useMutation({
    mutationFn: () =>
      api.resumeTaskGeneration(taskId, task?.active_job?.id || undefined),
    onSuccess: (res) => {
      const newJobId = (res as any)?.data?.id;
      if (newJobId) {
        activeJobIdRef.current = newJobId;
  ...
  const researchMutation = useMutation({
    mutationFn: () => {
      const persistedTopic = String(
        task?.detail?.topic || task?.title || "",
      ).trim();
    ...
    onError: (err: any) => {
      toast(
        `全网调研失败：${errorMessage(err, "请检查事实检索 Provider 配置")}`,
        "error",
      );
    },
  });
  ...
  const generateScriptMutation = useMutation({
    ...
    onError: (err: any) => {
      setMissingConfigAlert(
        err.message || "生成脚本失败，请检查大语言模型 Provider 配置。",
      );
    },
  });

  const applyScriptMutation = useMutation({
    mutationFn: (script: StructuredScript) =>
      api.applyScriptToTask(taskId, script),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["task-detail", taskId] });
      setIsAIOpen(false);
  ...
  const handleGenerateTTS = async (sceneId: string, index: number) => {
    ...
    try {
      if (task?.scenes && task.scenes[index]?.id) {
        await api.generateSceneTTS(
          task.scenes[index].id,
          task.generation_settings?.voice_id,
          Number(task.generation_settings?.speed || 1),
        );
        queryClient.invalidateQueries({ queryKey: ["task-detail", taskId] });
      } else {
      ...
      setMissingConfigAlert(
        e.message || "配音生成失败，请检查语音合成 Provider 配置。",
      );
    } finally {
  ...
  const handleGenerateImage = async (sceneId: string, index: number) => {
    ...
    try {
      ...
      setMissingConfigAlert(
        e.message || "画面生成失败，请检查图像生成 Provider 配置。",
      );
    } finally {
  ...
  const handleGenerateVideo = async (sceneId: string, index: number) => {
    ...
    try {
      ...
      setMissingConfigAlert(
        e.message || "视频生成失败，请检查视频生成 Provider 配置。",
      );
    } finally {
  ...
  const totalWords = scenes.reduce(
    (acc, sc) => acc + (sc.narration_text?.length || 0),
    0,
  );
  ...
  const persistedProgress = Math.max(
    ...
    Math.min(
      100,
      Number(task?.active_job?.progress ?? 0),
    ),
  );
  const liveProgressForCurrentJob =
    liveProgress !== null &&
    (!task?.active_job?.id ||
      !liveJobIdRef.current ||
      liveJobIdRef.current === task.active_job.id)
      ? liveProgress
      : null;
  ...
  const finalVideoUrl = liveVideoUrl || task?.active_job?.result?.final_video_url;
  ...
  const taskMetadata = task?.generation_settings?.metadata as
    PlatformMetadata | undefined;
  const scheduledPublish =
    task?.scheduled_publish || task?.publishing_settings?.scheduled_publish;
  const scheduledPublishStatus = String(scheduledPublish?.status || "pending");
  const canCancelScheduledPublish = [
    "pending",
    "scheduled",
    "queued",
    "publishing",
  ].includes(scheduledPublishStatus);
  const projectAspect = project?.aspect_ratio || "9:16";
  const availableTemplates = React.useMemo(() => {
    return templates.filter((item) => {
      const matchesMode = item.supported_content_modes.includes(
        taskContentMode as any,
      );
      const matchesAspect =
  ...
  const taskContentModeLabel =
    CONTENT_MODE_LABELS[taskContentMode] || taskContentMode;

  React.useEffect(() => {
    if (isRerenderOpen && availableTemplates.length > 0) {
      const exists = availableTemplates.some(
        (t) => t.id === rerenderTemplateId,
      );
      if (!exists) {
  ...
  const coverAssets = projectAssets.filter(
  ...
  const readySceneCount = scenes.filter((_, index) => {
    ...
    return Boolean(
      ...
      (scene?.media_asset_id || taskContentMode === "static"),
    );
  }).length;
    ...
    >
      ...
      <PageHeader
        back={
          <Link
            ...
            onClick={(event) => {
              if (
                isDirtyRef.current &&
                !window.confirm("当前故事板有未保存修改，确定离开吗？")
              )
                event.preventDefault();
            }}
        ...
        title={
          <span className="flex min-w-0 items-center gap-2">
            ...
            <StatusBadge
              label={TASK_STATUS_LABELS[currentStatus] || currentStatus}
              tone={getTaskStatusTone(currentStatus)}
            />
          </span>
        }
        description={
          taskContentMode === "online_asset"
            ...
            : taskContentMode === "uploaded_asset"
              ...
              : taskContentMode === "static"
                ...
                : taskContentModeSpec?.description ||
                  "调优分镜脚本、画面与配音，控制视频渲染流水线。"
        }
        actions={
          <div className="flex items-center gap-2">
            <span className="text-xs font-mono text-muted-foreground">
              {scenes.length} 个分镜 · {currentProgress}%
            </span>
            <Button
              variant="outline"
              size="sm"
      ...
      <div className="sticky top-0 z-20 w-full rounded-xl glass-panel p-3 border border-border/80 shadow-xs space-y-2.5 backdrop-blur-xl bg-card/85 dark:bg-card/75">
        ...
        <WorkflowMiniRail
          ...
          onResume={
            task?.can_resume ? () => resumeMutation.mutate() : undefined
          }
          isResumePending={resumeMutation.isPending}
          activeStageKey={String(
            task?.current_stage || task?.active_job?.current_stage || "",
          ).toLowerCase()}
          variant="integrated"
        />
          ...
          <div className="flex flex-wrap items-center gap-2">
            {primaryAction === "save" && (
              <Button
                ...
                disabled={
                  saveScenesMutation.isPending || updateTaskMutation.isPending
                }
            ...
            {primaryAction === "generate" && (
              <Button
                onClick={() => {
                  if (requireSaved()) generateWorkflowMutation.mutate();
                }}
                disabled={generateWorkflowMutation.isPending}
                className="gap-1.5 h-9 px-4 text-sm font-medium shadow-sm"
              >
            ...
            {primaryAction === "compose" && (
              <Button
                onClick={() => {
                  if (requireSaved()) composeVideoMutation.mutate();
                }}
                disabled={
                  composeVideoMutation.isPending ||
                  isRunning ||
                  scenes.length === 0 ||
                  isDirty
                }
            ...
            {primaryAction === "publish" && (
              <Button
                onClick={() => {
                  if (requireSaved()) setIsPublishOpen(true);
                }}
                disabled={!finalVideoUrl || isDirty}
                className="gap-1.5 h-9 px-4 text-sm font-medium shadow-sm"
              >
            ...
            <div ref={moreMenuRef} className="relative inline-block">
              ...
              >
                ...
                <ChevronDown
                  aria-hidden="true"
                  className={`h-3.5 w-3.5 transition-transform duration-150 ${isMoreMenuOpen ? "rotate-180" : ""}`}
                />
              </Button>
              {isMoreMenuOpen && (
                <div
                ...
                >
                  <Button
                    ...
                    onClick={() => {
                      setIsMoreMenuOpen(false);
                      if (requireSaved()) {
                        const activeMatch = availableTemplates.some(
                          (t) => t.id === renderTemplateId,
                        );
                        const targetId = activeMatch
                          ? renderTemplateId
                          : availableTemplates[0]?.id || renderTemplateId;
                        const targetTpl =
                          availableTemplates.find((t) => t.id === targetId) ||
                          templates.find((t) => t.id === targetId);
                        setRerenderTemplateId(targetId);
                        setRerenderParams(
                          activeMatch
                            ? renderParams
                            : targetTpl?.default_params || {},
                        );
            ...
            <div className="flex items-center gap-2 shrink-0 self-end sm:self-start">
              ...
              <Link href="/settings">
                <Button
                  size="sm"
                  variant="outline"
                  className="gap-1 text-xs h-8 border-destructive/30 text-destructive hover:bg-destructive-soft"
                >
              ...
              <div className="font-medium text-foreground flex items-center gap-2 flex-wrap">
                ...
                {missingItems.length - criticalMissing.length > 0 && (
                  <Badge variant="warning" className="text-xs">
                    {missingItems.length - criticalMissing.length} 项 Provider
                    需关注 · {pendingTestItems.length} 项待测试
                  </Badge>
                )}
              </div>
            </div>
          ...
          {showReadinessDetails && (
            <div className="pt-2 border-t border-border space-y-2">
              <div className="text-xs text-muted-foreground">
                已保存但未测试的 Provider 会单独标记；缺少关键 Provider
                时，相关生成环节将无法执行。
              </div>
        ...
        <Card className="border-primary/25 bg-primary-soft/40">
          <CardContent className="flex flex-col gap-3 py-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex min-w-0 items-start gap-2.5">
              ...
              <div className="min-w-0 space-y-1">
                <div className="flex flex-wrap items-center gap-2 text-sm font-semibold text-foreground">
                  ...
                  <StatusBadge
                    label={
                      PUBLISH_STATUS_LABELS[scheduledPublishStatus] ||
            ...
            {canCancelScheduledPublish && (
              ...
              >
                {cancelScheduledPublishMutation.isPending ? (
                  <Loader2 className="h-3.5 w-3.5 animate-spin" />
                ) : (
      ...
      {task.detail.type === "commerce" && (
        <CommercePreflightCard taskId={taskId} busy={isRunning} />
      )}
          ...
          <Card className="flex flex-col justify-between overflow-hidden">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                ...
                {finalVideoUrl && <StatusBadge label="已就绪" tone="success" />}
              </div>
            </CardHeader>
            <CardContent className="space-y-3">
              {finalVideoUrl ? (
                <div className="space-y-3">
                  <div
                    className={`${videoAspectClass} mx-auto bg-black rounded-lg overflow-hidden flex items-center justify-center border border-border shadow-sm`}
                  >
                    <video
                      key={finalVideoUrl}
                      src={finalVideoUrl}
                ...
                <div
                  className={`${videoPlaceholderAspectClass} mx-auto bg-secondary/40 rounded-lg border border-dashed border-border flex flex-col items-center justify-center p-4 text-center text-muted-foreground animate-pulse`}
                >
                  <Loader2 className="h-7 w-7 mb-2 animate-spin text-primary" />
                  <p className="text-xs font-medium text-foreground">
                    {liveStepMessage || "正在重新合成视频…"}
                  </p>
                ...
                <div
                  className={`${videoPlaceholderAspectClass} mx-auto bg-secondary/40 rounded-lg border border-dashed border-border flex flex-col items-center justify-center p-4 text-center text-muted-foreground`}
                >
            ...
            <div className="border-t border-border/60 px-3 py-2.5">
              <Button
                ...
                className="gap-1.5"
                disabled={
                  isRunning || isDirty || regenerateMetadataMutation.isPending
                }
            ...
            {taskMetadata && (
              <div className="border-t border-border/60 px-3 py-2.5 space-y-1.5 text-xs">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-muted-foreground">
                    {PLATFORM_LABELS[taskMetadata.platform || "douyin"] ||
                      taskMetadata.platform ||
                      "抖音"}
                  </span>
                </div>
                ...
                {Array.isArray(taskMetadata.tags) &&
                  taskMetadata.tags.length > 0 && (
            ...
            <Card>
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge
                      variant={isTaskEventsConnected ? "success" : "warning"}
                      className="text-xs"
                    >
                  ...
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground pt-1">
                    ...
                    {(task.resume_count || task.active_job.retry_count) > 0 && (
                      <Badge variant="outline" className="text-xs">
                        重试：{task.resume_count || task.active_job.retry_count}
                        /{task.active_job.max_retries}
                      </Badge>
                    )}
                    {liveCurrentScene && liveTotalScenes && (
                      <span>
                        当前分镜：{liveCurrentScene}/{liveTotalScenes}
                      </span>
                    )}
                    {task.can_resume && (
                      <Badge variant="outline" className="text-xs">
            ...
            <Card>
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between gap-3">
                  ...
                  {isDirty && (
                    <span className="text-xs text-warning">待保存</span>
                  )}
                </div>
              </CardHeader>
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className="text-center text-xs text-muted-foreground font-mono">
                    {selectedTemplate?.width ||
                      (projectAspect === "16:9" ? 1920 : 1080)}{" "}
                    ×{" "}
                    {selectedTemplate?.height ||
                      (projectAspect === "16:9" ? 1080 : 1920)}
                  </span>
                </div>
              </CardContent>
            </Card>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            className="gap-1.5 text-sm h-9 px-3.5"
          >
        ...
        <div className="space-y-3">
          {scenes.map((scene, index) => {
            ...
            const hasVideo = Boolean(
              dbScene?.rendered_segment_asset_id ||
              (dbScene?.media_asset_id &&
                dbScene?.layout_params?.media_type === "video"),
            );
            const contentMode =
              task?.generation_settings?.content_mode || "generated_image";
            const sceneAsset = projectAssets.find(
              (asset) => asset.id === dbScene?.media_asset_id,
            );
            const sceneMediaType =
              dbScene?.layout_params?.media_type || sceneAsset?.asset_type;
                  ...
                  <div className="p-4 flex-1 space-y-3">
                      ...
                      dbScene && (
                        <div className="flex flex-wrap items-center gap-3 rounded-md border border-border/60 bg-secondary/15 p-2.5">
                          {(hasImage ||
                            ...
                            contentMode === "static") && (
                            <div
                              className={`relative ${sceneThumbAspectClass} overflow-hidden rounded-md border border-border bg-black shrink-0`}
                            >
                              {contentMode === "static" ? (
                                <img
                                  src={`/api/v1/templates/previews/${encodeURIComponent(renderTemplateId)}`}
                    ...
                    <div className="space-y-2 rounded-lg border border-primary/15 bg-primary/[0.03] p-3">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                        <div className="space-y-1.5">
                          <label
                            htmlFor={`scene-${index}-visual-role`}
                          ...
                          <Select
                            ...
                            onChange={(event) =>
                              handleSceneChange(
                                index,
                                "visual_role",
                                event.target.value as VisualRole,
                              )
                            }
                        ...
                        <div className="space-y-1.5">
                          <label
                            htmlFor={`scene-${index}-claim-refs`}
                            className="text-xs font-medium text-muted-foreground"
                          >
                            对应主张 ID
                          </label>
                        ...
                        <div className="space-y-1.5">
                          <label
                            htmlFor={`scene-${index}-source-refs`}
                            className="text-xs font-medium text-muted-foreground"
                          >
                            来源 ID
                          </label>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
                      ...
                      <div
                        className={`${usesVisualPrompt ? "sm:col-span-2" : "sm:col-span-4"} space-y-1.5`}
                      >
                        <label
                          htmlFor={`scene-${index}-duration`}
                          className="text-xs font-medium text-muted-foreground"
                        >
                          时长（秒）
                        </label>
                      ...
                      <div
                        className={`${usesVisualPrompt ? "sm:col-span-4" : "sm:col-span-8"} space-y-1.5`}
                      >
                        <label
                          htmlFor={`scene-${index}-asset`}
                          className="text-xs font-medium text-muted-foreground"
                        >
                          {contentMode === "uploaded_asset"
                            ? "本镜头素材"
                        ...
                        <Select
                          ...
                          className="text-sm h-9"
                          onChange={(e) => {
                            const selected = projectAssets.find(
                    ...
                    <ScenePipelineStatus
                      ...
                      isUnitRetrying={Boolean(
                        retryingUnits[`voice-${dbScene?.id}`] ||
                        retryingUnits[`assets-${dbScene?.id}`],
                      )}
                      onGenerateTTS={() =>
                        handleGenerateTTS(dbScene?.id || "", index)
                      }
                      onGenerateImage={() =>
                        handleGenerateImage(dbScene?.id || "", index)
                      }
                      onGenerateVideo={() =>
                        handleGenerateVideo(dbScene?.id || "", index)
                      }
                      ...
                      canGenerateVisual={Boolean(
                        scene.visual_prompt || scene.narration_text,
                      )}
                      measuredDuration={
                        Number(
                          dbScene?.layout_params?.video_actual_duration_seconds,
                        ) || null
                      }
                      errorMessage={
                        dbScene?.layout_params?.image_error ||
                        dbScene?.layout_params?.video_error ||
                        dbScene?.layout_params?.tts_error ||
                        null
                      }
                    />
                  </div>
      ...
      <Dialog
        ...
        className="max-w-2xl max-h-[88vh]"
      >
        <DialogHeader>
          <DialogTitle>AI 知识脚本生成</DialogTitle>
          <DialogDescription>
            基于主题、Knowledge Brief
            和来源，生成带视觉角色与出处关系的结构化分镜。
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          ...
          {!llmConfigured && (
            <div className="space-y-1.5 rounded-md border border-warning/30 bg-warning-soft p-3 text-sm text-warning">
              ...
              <p className="text-xs leading-5 text-muted-foreground">
                AI 剧本创作需要语言模型 Provider。请前往设置中心配置大语言模型
                (LLM)，也可以关闭窗口手动录入台词与画面提示词。
              </p>
              <Link href="/settings" className="inline-block pt-1">
                <Button
                  size="sm"
                  variant="outline"
                  className="gap-1 border-warning/40 text-warning hover:bg-warning-soft"
                >
          ...
          <div className="space-y-1">
            ...
            {!searchConfigured && (
              <p className="flex items-center gap-1 pt-0.5 text-xs text-muted-foreground">
                ...
                />
                未配置事实检索
                Provider：全网调研会跳过外部检索，直接基于语言模型已有知识生成。
              </p>
            )}
            {taskResearch && (
              <div
                ...
                onChange={(e) =>
                  setAiSceneCount(
                    Math.min(
                      ...
                      Math.max(
                        SCENE_COUNT_MIN,
                        parseInt(e.target.value) || SCENE_COUNT_MIN,
                      ),
                    ),
                  )
                }
          ...
          {researchData && (
            <div className="p-3 bg-secondary/50 rounded-md space-y-1.5 text-xs">
              ...
              <p className="text-xs text-muted-foreground">
                检索 Provider：{researchData.provider || "未配置"} · 状态：
          ...
          <Button
            ...
            onClick={() =>
              generateScriptMutation.mutate({
                ...
                knowledge_brief: task?.detail as unknown as
                  KnowledgeBrief | undefined,
                research_sources:
                  researchData?.sources || taskResearch?.sources || [],
              })
            }
            ...
            <div className="p-3.5 bg-secondary/30 rounded-lg border border-border space-y-3 text-xs">
              ...
              {generatedScript.knowledge_brief && (
                <div className="space-y-1.5 pt-2 border-t border-border/40">
                  ...
                  <p className="text-foreground">
                    主张：
                    {generatedScript.knowledge_brief.thesis || "将由脚本整理"}
                  </p>
                  <p className="text-muted-foreground">
                    带走：
                    {generatedScript.knowledge_brief.viewer_takeaway ||
                      "将由脚本整理"}
                  </p>
                  {generatedScript.knowledge_brief.source_refs?.length > 0 && (
                    <p className="font-mono text-[11px] text-primary">
                      来源：
                      {generatedScript.knowledge_brief.source_refs.join(", ")}
                    </p>
                  )}
                </div>
              )}
              ...
              <div className="space-y-1.5 pt-2 border-t border-border/40">
                ...
                {generatedScript.scenes.map((sc, i) => (
                  <div
                    key={i}
                    className="p-2 bg-background rounded border border-border/60 space-y-1"
                  >
                    <div className="flex justify-between font-medium">
                      <span>
                        ...
                        {sc.visual_role && (
                          ...
                          >
                            {VISUAL_ROLE_OPTIONS.find(
                              (role) => role.value === sc.visual_role,
                            )?.label || sc.visual_role}
                          </Badge>
                        )}
                      </span>
                    </div>
                    ...
                    {sc.claim_refs?.length || sc.source_refs?.length ? (
                      <p className="font-mono text-[11px] text-primary">
                        {[
                          ...(sc.claim_refs || []).map((ref) => `主张:${ref}`),
                          ...(sc.source_refs || []).map((ref) => `来源:${ref}`),
                        ].join(" · ")}
                      </p>
                    ) : null}
      ...
      <Dialog
        open={isRerenderOpen}
        onClose={() => setIsRerenderOpen(false)}
        className="max-w-xl"
      >
        <DialogHeader>
        ...
        <div className="space-y-3.5 py-2">
            ...
            {(() => {
              ...
              return (
                <div className="flex items-center gap-3 rounded-xl glass-pill p-3 border border-border/70 shadow-xs">
                  <div
                    className={`${thumbAspect} shrink-0 overflow-hidden rounded-lg border border-border/80 bg-black`}
                  >
                    <img
                  ...
                  <div className="flex-1 min-w-0 space-y-1 text-xs">
                    <div className="font-semibold text-foreground truncate">
                      {formatTemplateName(
                        currentRerenderTpl.id,
                        currentRerenderTpl.name,
                      )}
                    </div>
                    <div className="text-xs font-mono text-muted-foreground">
                      规格: {currentRerenderTpl.width}×
                      {currentRerenderTpl.height} ·{" "}
                      {currentRerenderTpl.aspect_ratio || "9:16"}
                    </div>
          ...
          {(
            availableTemplates.find((item) => item.id === rerenderTemplateId) ||
            templates.find((item) => item.id === rerenderTemplateId)
          )?.parameter_schema.map((parameter) => (
            <div key={parameter.name} className="space-y-1.5">
              <label
                htmlFor={`rerender-param-${parameter.name}`}
                className="text-xs text-muted-foreground"
              >
              ...
              <Input
                ...
                value={String(
                  rerenderParams[parameter.name] ?? parameter.default ?? "",
                )}
                onChange={(e) =>
                  setRerenderParams((current) => ({
                    ...current,
          ...
          <div className="space-y-2 border-t border-border/50 pt-3">
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <span>音量</span>
              <label htmlFor="rerender-bgm-volume" className="sr-only">
                背景音乐音量
              </label>
              <input
        ...
        <DialogFooter>
          <Button
            type="button"
            variant="outline"
            onClick={() => setIsRerenderOpen(false)}
          >
            取消
          </Button>
          <Button
        ...
        <div className="space-y-4 py-2">
          ...
          >
            ...
            {publishingAccounts.length === 0 && (
              <p className="mt-1 text-xs text-muted-foreground">
                请先在设置中心完成 QR 扫码授权，发布时会使用托管的 Cookie。
              </p>
            )}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <Select
              ...
              value={publishMode}
              onChange={(e) =>
                setPublishMode(e.target.value as "now" | "schedule")
              }
            >
      ...
      <ConfirmDialog
        ...
        description={
          sceneToDelete
            ? `“${sceneToDelete.label}”的台词、${isSourceMaterialMode(taskContentMode) ? "来源素材匹配" : taskContentMode === "uploaded_asset" ? "我的素材绑定" : taskContentMode === "static" ? "文字排版内容" : "画面提示词"}和排序将从当前故事板中移除。`
            : undefined
        }
        confirmLabel="确认删除"
        variant="destructive"
      ...
      <ConfirmDialog
        ...
        title="应用新脚本？"
        description={
          isSourceMaterialMode(taskContentMode)
            ? "当前故事板中的旁白台词会被新脚本替换，并重新匹配来源素材。"
            : taskContentMode === "uploaded_asset"
              ? "当前故事板中的旁白台词会被新脚本替换，保留我的素材绑定。"
              : taskContentMode === "static"
                ? "当前故事板中的旁白台词会被新脚本替换，保留文字排版。"
                : "当前故事板中的旁白和画面提示词会被新脚本替换。"
        }
        confirmLabel="确认应用"
        onConfirm={async () => {
          if (generatedScript)
            await applyScriptMutation.mutateAsync(generatedScript);
        }}
      />
