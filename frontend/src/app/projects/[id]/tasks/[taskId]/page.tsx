...
export default function TaskEditorPage() {
  ...
  const retrySceneMutation = useMutation({ mutationFn: (sceneId: string) => api.retrySceneMedia(taskId, sceneId), onSuccess: async (job) => { setSelectedJobId(job.id); await refresh(); toast("已按原 Snapshot 创建单镜重试；完成后仍需重新合成最终视频。", "success"); }, onError: fail("单镜重试失败") });
