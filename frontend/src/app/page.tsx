  ...
  return (
    <PageContainer width="wide" className="space-y-6">
      ...
      <section className="space-y-3" aria-labelledby="production-modes-title">
        <SectionHeader
          title={<span id="production-modes-title">选择内容赛道</span>}
        />
          ...
          {isProjectsLoading ? (
            <div className="grid gap-3.5 sm:grid-cols-2">
              {[1, 2].map((item) => (
                <div
                  key={item}
                  className="h-36 animate-pulse rounded-xl border border-border/60 bg-card/40"
                />
              ))}
            </div>
          ) : projects.length === 0 ? (
            <EmptyState
              icon={FolderKanban}
              title="暂无项目"
              description="创建项目后，排版模板和视频任务会集中在此管理。"
            />
          ) : (
            <div className="grid gap-3.5 sm:grid-cols-2">
              {projects.map((project) => (
                <Card
                  key={project.id}
                  className="group flex flex-col justify-between hover:border-primary/40 transition-all duration-150"
                >
                  ...
                  <CardContent className="flex items-center justify-between gap-3 border-t border-border/60 px-4 sm:px-5 py-3 text-xs sm:text-sm">
                    <span className="truncate text-muted-foreground">
                      模板：
                      <strong className="text-foreground/85 font-normal">
                        {formatTemplateName(
                          String(project.default_production_settings?.template_id || ""),
                          String(project.default_production_settings?.template_name || ""),
                        ) || "默认模板"}
                      </strong>
                    </span>
                    <Link
        ...
        <section className="space-y-3" aria-labelledby="ready-videos-title">
          ...
          {completedTasks.length === 0 ? (
            <EmptyState
              icon={Film}
              title="暂无成片"
              description="完成视频合成后，MP4 成片会展示在此处。"
            />
          ) : (
            <div className="space-y-3">
              {completedTasks.slice(0, 4).map((task) => (
                <Card
                  key={task.id}
                  className="flex items-center justify-between gap-3.5 p-4 hover:border-success/40 transition-all duration-150"
                >
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span
                        aria-hidden="true"
                        className="h-2 w-2 rounded-full bg-success shadow-[0_0_6px_hsl(var(--success))] shrink-0"
                      />
      ...
      >
        <DialogHeader>
          <DialogTitle className="truncate pr-6 text-base sm:text-lg font-semibold">
            {previewVideo?.title || "成片预览"}
          </DialogTitle>
