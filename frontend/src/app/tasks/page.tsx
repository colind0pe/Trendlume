...
export default function TasksGlobalPage() {
  ...
  const filteredTasks = React.useMemo(() => {
    return tasks.filter((task) => {
      ...
      const matchesSearch =
        ...
        (task.error_message && task.error_message.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const filters = [
    ...
    { label: "运行中", value: "running", count: tasks.filter((task) => isTaskActive(task)).length },
    { label: "已完成", value: "completed", count: tasks.filter((task) => task.status === "completed").length },
    { label: "失败", value: "failed", count: tasks.filter((task) => task.status === "failed").length },
    { label: "草稿", value: "draft", count: tasks.filter((task) => task.status === "draft" || task.status === "pending").length },
  ];
    ...
    <PageContainer width="wide" className="space-y-5">
      <PageHeader
        ...
        actions={(
          <div className="flex items-center gap-2">
            <div className="inline-flex items-center rounded-xl glass-pill p-0.5 border border-border/60 shadow-xs" role="group" aria-label="任务显示方式">
              <button
                type="button"
                aria-label="卡片视图"
              ...
              <button
                ...
                className={`inline-flex h-8 w-8 items-center justify-center rounded-lg text-muted-foreground transition-all cursor-pointer ${
                  viewMode === "table" ? "bg-card text-foreground shadow-xs font-medium" : "hover:text-foreground hover:bg-card/40"
                }`}
              >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <Tabs value={statusFilter} onValueChange={setStatusFilter} className="space-y-0">
          <TabsList aria-label="任务状态" className="h-9 max-w-full overflow-x-auto no-scrollbar">
            {filters.map((filter) => (
              <TabsTrigger key={filter.value} value={filter.value} className="gap-1.5 text-sm h-8 px-3">
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
          {[1, 2, 3, 4].map((item) => (
            <div key={item} className="h-48 animate-pulse rounded-xl border border-border/60 bg-card/40" />
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
          {filteredTasks.map((task) => {
            ...
            return (
              <Card key={task.id} className="group flex flex-col justify-between glass-card rounded-xl hover:border-primary/40 hover:shadow-glass-hover transition-all duration-200">
                <CardHeader className="p-4 sm:p-5 pb-3">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    />
                    <span className="font-mono text-xs tabular-nums text-muted-foreground/80">{formatDate(task.created_at)}</span>
                  </div>
                ...
                <CardContent className="space-y-3 border-t border-border/60 p-4 sm:p-5 py-3 text-sm text-muted-foreground">
                  ...
                  {task.status === "failed" && task.error_message && (
                    <div className="flex gap-2 rounded-lg border border-destructive/20 bg-destructive/5 p-2.5 text-xs text-destructive">
                      ...
                      <span className="line-clamp-2 font-mono">{task.error_message}</span>
                    </div>
                  )}
                ...
                <CardFooter className="p-4 sm:p-5 pt-0">
                  <Link href={`/projects/${task.project_id}/tasks/${task.id}`} className="w-full">
                    <Button variant="outline" size="sm" className="w-full justify-between group/btn h-9 px-3.5 text-sm">
                      <span>进入分镜工坊</span>
            ...
            <table className="w-full text-left text-sm border-collapse">
              <thead>
                <tr className="border-b border-border/70 bg-secondary/40 text-muted-foreground text-xs uppercase tracking-wider">
                  ...
                  <th className="py-3 px-4 font-semibold text-center">分镜数</th>
                    ...
                    <tr key={task.id} className="hover:bg-secondary/30 transition-colors">
                      ...
                      <td className="py-3.5 px-4 min-w-[220px] max-w-[340px]">
                        ...
                        {task.error_message && task.status === "failed" ? (
                          <p className="text-xs text-destructive truncate font-mono mt-1">{task.error_message}</p>
                        ) : task.description ? (
                      ...
                      <td className="py-3.5 px-4 min-w-[160px]">
                        {active ? (
                          ...
                          <span className="text-xs text-muted-foreground font-mono">
                            {task.status === "completed" ? "已完成全自动合成" : task.status === "failed" ? "已中断" : "待启动"}
                          </span>
                        )}
                      </td>
                      ...
                      <td className="py-3.5 px-4 text-right whitespace-nowrap">
                        <Link href={`/projects/${task.project_id}/tasks/${task.id}`}>
                          <Button size="sm" variant="outline" className="gap-1 h-8 px-3 text-xs sm:text-sm">
                            <span>进入</span>
                            <ArrowRight aria-hidden="true" className="h-3.5 w-3.5" />
                          </Button>
                        </Link>
